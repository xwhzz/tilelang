...
#define TVM_DECLARE_SYMBOLIC_MATCHER_BINOP(OpName)                       \
  ...
  void VisitExpr_(const CallNode* op, const PrimExpr& other) {
    ...
    if (rhs && op->op.same_as(rhs->op) && op->args.size() == rhs->args.size()) {
      for (size_t i = 0; i < op->args.size(); ++i) {
        VisitExpr(op->args[i], rhs->args[i]);
      }
    } else {
      must_prove_ = must_prove_ && (ffi::GetRef<PrimExpr>(op) == other);
    }
...
class FusedTIRConstructor : public ExprVisitor {
 ...
 private:
  ...
  tir::PrimFunc ConstructFunc() {
    ...
    tl::FuseTIRBufferSubstitutor subst(func_info_.buffer_subst_map, func_info_.symbolic_var_remap);
    ...
    tir::Stmt body = tl::BlockNameDeduplicator()(tir::SeqStmt::Flatten(func_info_.bodies));

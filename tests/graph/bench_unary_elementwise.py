...
Type-promotion note
-------------------
TileLang's ``TileLangFXImporter`` auto-promotes transcendental pointwise ops
(exp, gelu, ...) to fp32 for fp16/bf16 inputs, matching Inductor's
``register_pointwise_numeric`` behaviour.  Tests therefore use plain
``torch.exp(x)`` / ``F.gelu(x)`` and rely on the importer to emit the
fp32 round-trip in the generated kernel.
"""
...
# ---------------------------------------------------------------------------
# Model definitions — one plain module per op; auto fp32 promotion in the
# TileLang importer takes care of the cast-up / cast-back for fp16/bf16 inputs.
# ---------------------------------------------------------------------------
...
_MODEL_BY_OP = {"exp": _Exp, "gelu": _Gelu}
...
def _get_model(op_name, backend):
    return _MODEL_BY_OP[op_name]().cuda().eval()
...
def main():
    ...
    try:
        for op_name, N, dtype in product(ops, SHAPES, DTYPES):
            ...
            ref_model = _MODEL_BY_OP[op_name]().cuda().eval()
            with torch.no_grad():

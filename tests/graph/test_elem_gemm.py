...
def test_elem_gemm():
    ...
    out = compiled(x, w2)
    torch.testing.assert_close(out, fn(x, w2), atol=0.01, rtol=0.01)

    eager_ms = profiler.do_bench(lambda: fn(x, w2))
    fused_ms = profiler.do_bench(lambda: compiled(x, w2))
    print(f"Elem GEMM eager:          {eager_ms:.4f} ms ({eager_ms * 1000:.1f} us)")
    print(f"Elem GEMM TileLang fused: {fused_ms:.4f} ms ({fused_ms * 1000:.1f} us)")
    print(f"Elem GEMM speedup:        {eager_ms / fused_ms:.2f}x")


if __name__ == "__main__":
    test_elem_gemm()
    print("PASS: test_elem_gemm")

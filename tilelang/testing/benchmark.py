...
def analyze_records(records, out_dir):
    ...
    print(tabulate(records, headers=headers, tablefmt="github", stralign="left", numalign="decimal"))
...
def _load_module(full_path):
    module_name = f"tilelang_bench_{abs(hash(os.path.abspath(full_path)))}"
    spec = importlib.util.spec_from_file_location(module_name, full_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Failed to load module spec for {full_path}")

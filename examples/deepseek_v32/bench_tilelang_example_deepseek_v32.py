...
def bench_topk_selector():
    tilelang.testing.benchmark.process_func(topk_selector.run_regression_perf)


def bench_fp8_lighting_indexer():
    tilelang.testing.benchmark.process_func(fp8_lighting_indexer.run_regression_perf, S=512, SKV=1024, H=32, HKV=1, D=64, kv_stride=1)


def bench_sparse_mla_fwd():
    tilelang.testing.benchmark.process_func(sparse_mla_fwd.run_regression_perf, S=256, SKV=1024, H=64, HKV=1, DQK=576, DV=512, topk=256)
...
def bench_sparse_mla_bwd():
    tilelang.testing.benchmark.process_func(
        sparse_mla_bwd.run_regression_perf, S=256, SKV=512, H=64, HKV=1, DQKV=576, DV=512, topk=256, check_correctness=False
    )
